// Mini pixel-pet animation for the Status window. The sprite gif handles its
// own per-frame animation, so this module just drives idle bounce, occasional
// walk across a small stage, and the occasional jump arc — all via CSS
// transform so it runs on the GPU compositor at 60fps.
...
const TICK_MS = 60 // ~16 fps target, plenty for sprite drift on a status panel
...
export class PetCanvas {
  ...
  setDigimon(digimonId: string, eggVariant: number | undefined): void {
    ...
    // Reset position so the new sprite starts centered.
    this.pos = { x: 0, y: 0 }
    this.dir = 1
    this.movement = 'idle'
  ...
  private tick(): void {
    this.tickCount++
    // Eggs don't hatch yet, so they don't wander or bounce — just sit there.
    if (this.isEgg()) {
      this.movement = 'idle'
      this.pos = { x: 0, y: 0 }
      this.jumpTicksLeft = 0
