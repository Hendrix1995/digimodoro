...
import type { Snapshot, AppConfig, GraveyardEntry } from '../api.js'
import { t, type Lang } from '@digimodoro/core'
import { PetCanvas } from './pet-canvas.js'

const api = window.digimodoro

// Same right-facing table as the pet window — duplicated here so the
// control's mini-pet flips correctly too.
const SPRITE_DEFAULT_FACING: Record<string, 'left' | 'right'> = {
  hanumon: 'right', chocomon: 'right', damemon: 'right', darkdramon: 'right',
  darktyranomon: 'right', gaogamon: 'right', dracumon: 'right', ebidramon: 'right',
  exogrimmon: 'right', geremon: 'right', gizamon: 'right', greymon: 'right',
  knightchessmonwhite: 'right', kunemon: 'right', reppamon: 'right',
  metaletemon: 'right', metalgarurumon: 'right', metalgreymon: 'right', numemon: 'right',
}
...
function evoThumbUrl(digimonId: string, eggVariant: number): string {
  if (digimonId === 'egg') {
    const key = 'v' + String(eggVariant).padStart(2, '0')
...
function graveThumbUrl(entry: GraveyardEntry): string {
  if (entry.digimonId === 'egg') {
    const v = entry.seedEggVariant ?? 1
    return `${spriteBase}egg/v${String(v).padStart(2, '0')}.png`
  }
  return `${spriteBase}${entry.digimonId}/idle.gif`
}

function makeGraveThumb(src: string): HTMLImageElement {
  const img = document.createElement('img')
  img.className = 'grave-thumb'
  img.alt = ''
  img.src = src
  img.addEventListener('error', () => {
    img.style.visibility = 'hidden'
...
function renderGraveyard(): void {
  ...
  for (const entry of [...graveyard].reverse()) {
    ...
    const thumb = makeGraveThumb(graveThumbUrl(entry))
...
function buildGraveTree(entry: GraveyardEntry): HTMLElement {
  ...
  // Chain: first entry's `from` + every `to`. If no history, just show
  // the egg variant.
  ...
  ids.forEach((id, i) => {
    ...
    img.alt = ''
    img.src = id === 'egg'
      ? `${spriteBase}egg/v${String(eggVariant).padStart(2, '0')}.png`
      : `${spriteBase}${id}/idle.gif`
    img.addEventListener('error', () => {
      img.style.visibility = 'hidden'
    })
...
async function refreshGraveyard(): Promise<void> {
  try {
    graveyard = await api.getGraveyard()
  } catch {
    graveyard = []
  }
  renderGraveyard()
...
function applySnapshot(s: Snapshot): void {
  ...
  for (const slot of ['morning', 'forenoon', 'midday', 'evening', 'night'] as const) {
    ...
    chip.innerHTML = '<strong>' + counts[slot] + '</strong>' + t('slot_' + slot, lang)
  ...
  if (s.state.evolutionHistory.length === 0) {
    els.evoList.innerHTML = '<li class="evo-empty">—</li>'
  } else {
    els.evoList.innerHTML = ''
...
els.btnPrimary.addEventListener('click', async () => {
  ...
  if (a === 'start') await api.startFocus()
  else if (a === 'pause') await api.pause()
  else if (a === 'resume') await api.resume()
  else if (a === 'abort') {
    if (window.confirm(t('confirmAbort', lang))) await api.abortFocus()
  } else if (a === 'ack-done') await api.ackDone()
  else if (a === 'skip-break') await api.skipBreak()
  else if (a === 'reset') await api.resetPet()
})
els.btnSecondary.addEventListener('click', async () => {
  const a = els.btnSecondary.dataset.action
  if (a === 'skip-break') await api.skipBreak()
  else if (a === 'abort') {
    if (window.confirm(t('confirmAbort', lang))) await api.abortFocus()
  }
})
els.btnReset.addEventListener('click', async () => {
  if (window.confirm(t('confirmReset', lang))) await api.resetPet()
})
...
els.segLang.addEventListener('click', async (e) => {
  ...
  // Apply everything immediately — do not wait for the main process roundtrip.
  cfg.ui.language = l
  lang = l
...
els.segSize.addEventListener('click', async (e) => {
  ...
  if (!t || !cfg) return
  const s = Number(t.dataset.scale)
  cfg.ui.petScale = s
  setSegOn(els.segSize, '[data-scale="' + s + '"]')
  await api.setScale(s)
})
...
async function boot(): Promise<void> {
  ...
  api.onSnapshot((s) => applySnapshot(s))
  api.onLang((l) => {
    lang = l
    if (cfg) cfg.ui.language = l
    applyLangTexts()
    syncSettingsUi()
    if (snap) applySnapshot(snap)
  })
